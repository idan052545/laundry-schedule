...
export default function AppealsPanel({ appeals, submitting, onResolve }: AppealsPanelProps) {
  ...
  return (
    <div className="bg-red-50 rounded-2xl border border-red-200 p-4 mb-6 space-y-3">
      <h3 className="font-bold text-red-700 flex items-center gap-2 text-sm"><MdGavel /> {t.guardDuty.pendingDisputes}</h3>
      {pending.map(appeal => (
        <div key={appeal.id} className="bg-white rounded-xl p-3 border border-red-100 flex flex-col sm:flex-row sm:items-center gap-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 truncate">{appeal.reason || t.guardDuty.noReason}</p>
              {appeal.suggestedUser && (
                <span className="text-[10px] text-blue-600 font-medium">{t.guardDuty.suggests}: {appeal.suggestedUser.name}</span>
              )}
            </div>
          </div>
