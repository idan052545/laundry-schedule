    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-5 max-w-sm w-full shadow-xl space-y-4" onClick={e => e.stopPropagation()}>
        <h3 className="font-bold text-gray-800 flex items-center gap-2">
          <MdGavel className="text-red-500" /> {t.guardDuty.appealTitle}
        </h3>
        <div className="bg-red-50 rounded-xl p-3 text-xs space-y-1">
          <div>{t.guardDuty.role}: <strong>{appealing.role}</strong></div>
          <div>{t.guardDuty.shiftLabel}: <strong>{appealing.timeSlot}</strong></div>
        </div>
        <textarea value={appealReason} onChange={e => setAppealReason(e.target.value)}
          ...
          placeholder={t.guardDuty.appealReason} />
        <div>
          <label className="text-xs text-gray-500 font-medium block mb-1">{t.guardDuty.suggestReplacement}</label>
          <select value={appealSuggestion} onChange={e => setAppealSuggestion(e.target.value)}
            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm">
            <option value="">{t.guardDuty.noSuggestion}</option>
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button onClick={onAppeal} disabled={submitting}
            ...
            <MdNotifications /> {submitting ? t.guardDuty.sending : t.guardDuty.sendAppeal}
          </button>
        </div>
      </div>
    </div>
