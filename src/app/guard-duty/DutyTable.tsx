      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr>
                <th className="border-b border-r border-gray-200 bg-gray-50 px-2 sm:px-3 py-2.5 text-gray-500 text-start font-bold sticky right-0 z-10 min-w-[60px] sm:min-w-[80px]">
                  <MdAccessTime className="inline text-sm ms-1" />{t.guardDuty.shift}
                </th>
                {roles.map(r => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          {dayRoleAssignments.map(({ role, people }) => (
            <div key={role} className={`rounded-xl border-2 p-3 ${ROLE_COLORS[role] ? "" : "border-gray-200"}`}
              ...
              <h4 className="font-bold text-sm mb-2 flex items-center gap-2 text-teal-600">
                <MdSecurity /> {role} <span className="text-[10px] font-normal text-gray-400">({t.guardDuty.dailyRole})</span>
              </h4>
                  ...
                  <div key={a.id} className="flex items-center gap-2 bg-white rounded-lg px-3 py-2 border border-teal-100">
                    ...
                    {isRoni && (
                      <button onClick={() => onSwap(a)}
                        className="text-blue-500 hover:text-blue-700 me-auto">
      ...
      {squads.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 mb-6">
          <h3 className="font-bold text-gray-700 text-sm mb-3 flex items-center gap-2">
            <MdGroups className="text-indigo-500" /> {t.guardDuty.squads}
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr>
                  <th className="border-b border-gray-200 bg-indigo-50 px-3 py-2 text-indigo-700 font-bold text-start">{t.guardDuty.squadNumber}</th>
                  <th className="border-b border-gray-200 bg-indigo-50 px-3 py-2 text-indigo-700 font-bold text-start">{t.guardDuty.cadet} 1</th>
                  <th className="border-b border-gray-200 bg-indigo-50 px-3 py-2 text-indigo-700 font-bold text-start">{t.guardDuty.cadet} 2</th>
                  <th className="border-b border-gray-200 bg-indigo-50 px-3 py-2 text-indigo-700 font-bold text-start">{t.guardDuty.cadet} 3</th>
                </tr>
              </thead>
              <tbody>
                {squads.map((s, i) => (
