...
const SURVEY_TYPE_CONFIG: Record<string, { label: string; icon: typeof MdPoll }> = {
...
function SimulationsSurveys({ userId, commanderId, isCommander }: { userId: string; commanderId: string; isCommander: boolean }) {
  ...
  if (selectedSurvey) {
    ...
    if (selectedSurvey.type === "yes_no") { resultMap.set("yes", 0); resultMap.set("no", 0); }
    else options.forEach((_, i) => resultMap.set(String(i), 0));
    selectedSurvey.responses.forEach((r) => {
      const ans = JSON.parse(r.answer);
      if (selectedSurvey.type === "multi") (ans as number[]).forEach((i: number) => resultMap.set(String(i), (resultMap.get(String(i)) || 0) + 1));
    ...
    return (
      <div className="space-y-4">
        <button onClick={() => { setSelectedSurvey(null); setDetailTeamFilter(null); }} className="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1">
          ← חזרה לסקרים
        </button>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
          ...
          {editing ? (
            <div className="space-y-3 border-b pb-4">
              ...
              <div className="flex gap-2 justify-end">
                ...
                <button onClick={handleEdit} disabled={sending} className="px-4 py-1.5 text-sm bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition disabled:opacity-50">{sending ? "שומר..." : "שמור"}</button>
              </div>
            </div>
          ) : (
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-bold text-gray-800">{selectedSurvey.title}</h3>
                  {isCreator && <button onClick={() => startEdit(selectedSurvey)} className="text-gray-400 hover:text-gray-600"><MdEdit className="text-lg" /></button>}
                </div>
                ...
                <div className="flex items-center gap-2 mt-2 text-xs text-gray-400">
                  ...
                  <span>•</span>
                  <span className={`px-2 py-0.5 rounded-full font-medium ${selectedSurvey.team === 0 ? "bg-purple-50 text-purple-600 border border-purple-200" : "bg-blue-50 text-blue-600 border border-blue-200"}`}>
                    {selectedSurvey.team === 0 ? "כל הפלוגה" : `צוות ${selectedSurvey.team}`}
                  </span>
                </div>
              </div>
          ...
          {selectedSurvey.status === "active" && (
            <div className="border rounded-xl p-4 space-y-3">
              ...
              {selectedSurvey.type === "multi" && (
                <SimMultiSelect options={options} selected={myAnswer || []} onSubmit={(sel) => handleRespond(selectedSurvey.id, sel)} />
              )}
            </div>
          )}
          ...
          {isCreator && (
            <div className="flex flex-wrap gap-2 pt-3 border-t">
              {selectedSurvey.status === "active" ? (
                <button onClick={() => handleAction(selectedSurvey.id, "close")} className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 flex items-center gap-1"><MdLock /> סגור סקר</button>
              ) : (
                <button onClick={() => handleAction(selectedSurvey.id, "reopen")} className="text-xs px-3 py-1.5 rounded-lg border border-green-200 text-green-600 hover:bg-green-50 flex items-center gap-1"><MdLockOpen /> פתח מחדש</button>
              )}
              ...
              <button onClick={() => handleExport(selectedSurvey)} className="text-xs px-3 py-1.5 rounded-lg border border-blue-200 text-blue-600 hover:bg-blue-50 flex items-center gap-1"><MdDownload /> ייצוא</button>
              <button onClick={() => handleDelete(selectedSurvey.id)} className="text-xs px-3 py-1.5 rounded-lg border border-red-200 text-red-500 hover:bg-red-50 flex items-center gap-1"><MdDelete /> מחק</button>
            </div>
          )}
        </div>
      </div>
...
// Multi-select helper for SimulationsSurveys
function SimMultiSelect({ options, selected, onSubmit }: { options: string[]; selected: number[]; onSubmit: (sel: number[]) => void }) {
    ...
    <div className="space-y-2">
      {options.map((opt, i) => (
        <button key={i} type="button" onClick={() => toggle(i)}
          className={`w-full text-right p-3 rounded-lg text-sm transition flex items-center gap-2 ${local.includes(i) ? "bg-purple-600 text-white" : "bg-gray-50 hover:bg-gray-100 border border-gray-200"}`}>
            ...
            <button key={cmd.id} onClick={() => router.push(`/commander?id=${cmd.id}`)}
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-400 mt-1">{cmd._count.commanderPosts} פוסטים</div>
                </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-dotan-mint mb-4 sm:mb-6 space-y-3 sm:space-y-4">
          <div className="flex flex-wrap gap-2">
            {Object.entries(POST_TYPE_CONFIG).map(([key, { label, icon: Icon }]) => (
