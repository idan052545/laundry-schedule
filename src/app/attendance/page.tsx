...
export default function AttendancePage() {
  ...
  const deleteSession = async (sess: AttSession) => {
    if (!confirm(t.attendance.deleteConfirm.replace("{name}", sess.name))) return;
    ...
    <div>
        ...
        {showNewSession && isAuthorized && (
          <div className="flex gap-2 items-center">
            ...
            <button onClick={createSession}
              className="bg-dotan-green-dark text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-dotan-green transition">{t.attendance.create}</button>
          </div>
        )}
          ...
          <div className="flex flex-wrap gap-2">
            {sessions.map((sess) => (
              <div key={sess.id} className="flex items-center">
                ...
                {isAuthorized && (
                  <button onClick={() => deleteSession(sess)} className="text-red-400 hover:text-red-600 me-0.5 text-xs"><MdDelete /></button>
                )}
              </div>
            ))}
          </div>
      ...
      {loading && sessionsLoaded && selectedSession && (
        <div className="flex items-center justify-center py-12">
          <div className="text-gray-500">{t.attendance.loadingData}</div>
        </div>
      )}
      ...
      {sessionsLoaded && sessions.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          ...
          {isAuthorized && <p className="text-sm mt-2">{t.attendance.addHint}</p>}
        </div>
      )}
                  ...
                  {teamUsers.map((user) => {
                    ...
                    return (
                      <button key={user.id} onClick={() => toggleAttendance(user.id, isPresent)}
                        ...
                        className={`flex items-center gap-3 p-3 rounded-lg border-2 transition text-start ${
