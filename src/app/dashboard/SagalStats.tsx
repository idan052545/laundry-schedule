...
export default function SagalStats({ feed }: SagalStatsProps) {
  ...
  const catConfig = [
    { key: "kitchen", label: t.sagalStats.kitchen, icon: MdRestaurant, color: "text-orange-500" },
    { key: "cleaning", label: t.sagalStats.cleaning, icon: MdCleaningServices, color: "text-blue-500" },
    { key: "guard", label: t.sagalStats.guard, icon: MdSecurity, color: "text-red-500" },
    { key: "logistics", label: t.sagalStats.logistics, icon: MdLocalShipping, color: "text-purple-500" },
    { key: "general", label: t.sagalStats.general, icon: MdVolunteerActivism, color: "text-green-500" },
    { key: "other", label: t.sagalStats.other, icon: MdMoreHoriz, color: "text-gray-500" },
  ];
  ...
  return (
    <div className="mb-4 space-y-3">
      ...
      <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-200 rounded-xl px-3 py-2">
        ...
        <span className="text-xs font-bold text-indigo-700">{t.sagalStats.title}</span>
      </div>

      {/* Stats card */}
      <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm space-y-4">
        <h3 className="text-sm font-bold text-gray-700 flex items-center gap-1.5">
          <MdVolunteerActivism className="text-base text-indigo-500" /> {t.sagalStats.volunteerStats}
        </h3>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-indigo-50 rounded-xl px-3 py-2 text-center">
            ...
            <div className="text-[10px] text-indigo-500 font-medium flex items-center justify-center gap-0.5"><MdPeople className="text-xs" /> {t.sagalStats.activeRequests}</div>
          </div>
          <div className="bg-red-50 rounded-xl px-3 py-2 text-center">
            <div className="text-lg font-bold text-red-600">{urgentReqs}</div>
            <div className="text-[10px] text-red-500 font-medium flex items-center justify-center gap-0.5"><MdWarning className="text-xs" /> {t.sagalStats.urgent}</div>
          </div>
          <div className="bg-green-50 rounded-xl px-3 py-2 text-center">
            ...
            <div className="text-[10px] text-green-500 font-medium flex items-center justify-center gap-0.5"><MdAccessTime className="text-xs" /> {t.sagalStats.happeningNow}</div>
          </div>
          <div className="bg-amber-50 rounded-xl px-3 py-2 text-center">
            ...
            <div className="text-[10px] text-amber-500 font-medium flex items-center justify-center gap-0.5"><MdSchedule className="text-xs" /> {t.sagalStats.staffedSlots}</div>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] text-gray-500 font-medium">{t.sagalStats.overallStaffing}</span>
        ...
        {/* Category breakdown */}
        <div>
          <span className="text-[10px] text-gray-400 font-medium block mb-1.5">{t.sagalStats.categoryBreakdown}</span>
