...
import { getSectionLabels } from "./constants";
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-4 mb-4 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-bold text-gray-700">{t.dashboard.whatToShow}</span>
        <button onClick={onClose} className="text-xs text-gray-400 hover:text-gray-600">{t.common.close}</button>
      </div>
      {/* Style toggle */}
      <div className="flex gap-1 mb-3 bg-gray-100 rounded-xl p-1">
        {([["new", t.dashboard.cards], ["carousel", t.dashboard.carousel], ["classic", t.dashboard.classic]] as [DashStyle, string][]).map(([key, label]) => (
      ...
      {isRealAdmin && (
        <div className="mt-3 pt-3 border-t border-gray-200">
          <button onClick={() => { const next = !sagalMode; setSagalMode(next); localStorage.setItem("sagal-mode", String(next)); }}
            ...
            {sagalMode ? t.dashboard.sagalModeActive : t.dashboard.sagalModeBtn}
          </button>
        </div>
      )}
    </div>
