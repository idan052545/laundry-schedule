...
import { getFeedbackTypes } from "../constants";
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center" onClick={onClose}>
      <div className="bg-white w-full max-w-sm rounded-2xl p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
        <h2 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-2"><MdStar className="text-purple-500" /> {t.volunteers.feedbackTitle}</h2>
        ...
        <div className="flex gap-1.5 justify-center mb-3">
          {feedbackTypes.map(ft => (
            <button key={ft.value} onClick={() => setFeedbackForm(f => ({ ...f, type: ft.value }))}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition ${
                feedbackForm.type === ft.value ? "bg-purple-600 text-white border-purple-600" : "bg-gray-50 border-gray-200 text-gray-600"
              }`}>
              <ft.icon className="inline text-sm" /> {ft.label}
            </button>
          ))}
        </div>
        <textarea value={feedbackForm.comment} onChange={e => setFeedbackForm(f => ({ ...f, comment: e.target.value }))}
          placeholder={t.volunteers.feedbackComment} rows={2} className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm resize-none mb-3" />
        <div className="flex gap-2">
          <button onClick={() => onSubmit(showFeedback)} disabled={submitting || feedbackForm.rating === 0}
            ...
            {submitting ? t.common.sending : t.volunteers.sendRating}
          </button>
