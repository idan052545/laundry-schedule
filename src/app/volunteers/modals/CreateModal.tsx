    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-end sm:items-center justify-center" onClick={onClose}>
      <div className="bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2"><MdAdd className="text-green-600" /> {isCommander ? t.volunteers.createTitle : t.volunteers.helpRequestTitle}</h2>
        ...
        {!isCommander && (
          <p className="text-xs text-gray-400 mb-3 bg-green-50 border border-green-100 rounded-lg px-3 py-2">
            {t.volunteers.helpRequestDesc}
          </p>
        )}

        <div className="space-y-4">
          <div className="relative">
            <label className="text-xs font-medium text-gray-600 mb-1 block">{isCommander ? t.volunteers.volunteerName : t.volunteers.helpName}</label>
            <input value={form.title} onChange={e => { setForm((f: FormType) => ({ ...f, title: e.target.value })); setShowTitleSuggestions(true); }}
              onFocus={() => setShowTitleSuggestions(true)}
              placeholder={t.volunteers.namePlaceholder} className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:ring-2 focus:ring-green-300 transition" />
              ...
              <div className="absolute z-10 top-full mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-40 overflow-y-auto">
                {filteredSuggestions.map(s => (
                  <button key={s.id} onClick={() => { setForm((f: FormType) => ({ ...f, title: s.title, category: s.category })); setShowTitleSuggestions(false); }}
                    className="w-full text-start px-3 py-2 text-sm hover:bg-gray-50 flex items-center justify-between">
                    <span>{s.title}</span>
          ...
          <div>
            <label className="text-xs font-medium text-gray-600 mb-1 block">{t.volunteers.descriptionLabel}</label>
            <textarea value={form.description} onChange={e => setForm((f: FormType) => ({ ...f, description: e.target.value }))}
              rows={2} placeholder="..." className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm resize-none focus:ring-2 focus:ring-green-300 transition" />
          </div>

          <div>
            <label className="text-xs font-medium text-gray-600 mb-1 block">{t.volunteers.categoryLabel}</label>
          ...
          <div>
            <label className="text-xs font-medium text-gray-600 mb-1 block">{t.volunteers.whoNeeded}</label>
            <div className="flex gap-1.5 flex-wrap">
              {[
                { key: "all", label: t.teams.allPlatoon },
                { key: "team-14", label: t.teams.team14 },
                { key: "team-15", label: t.teams.team15 },
                { key: "team-16", label: t.teams.team16 },
                { key: "team-17", label: t.teams.team17 },
                { key: "mixed", label: t.teams.mixed },
              ].map(opt => (
                <button key={opt.key} onClick={() => setForm((f: FormType) => ({ ...f, target: opt.key }))}
                  className={`px-2.5 py-1.5 rounded-lg text-[11px] font-medium border transition ${
                    form.target === opt.key ? "bg-green-600 text-white border-green-600" : "bg-gray-50 border-gray-200 text-gray-600"
                  }`}>
                  {opt.label}
                </button>
              ))}
          ...
          {form.target === "mixed" && (
            <div className="bg-gray-50 rounded-xl border border-gray-200 p-3 space-y-2">
              <div className="text-[11px] font-medium text-gray-600">{t.volunteers.howManyPerTeam}</div>
              {[14, 15, 16, 17].map(team => {
                const detail = form.targetDetails.find(d => d.team === team);
                return (
                  <div key={team} className="flex items-center gap-2">
                    <span className="text-xs font-bold text-gray-700 w-16">{t.common.team} {team}</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            {form.target !== "mixed" && (
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">{t.volunteers.howManyVolunteers}</label>
            ...
            <div>
              <label className="text-xs font-medium text-gray-600 mb-1 block">{t.volunteers.priorityLabel}</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="min-w-0">
              <label className="text-xs font-medium text-gray-600 mb-1 block">{t.volunteers.startLabel}</label>
            ...
            <div className="min-w-0">
              <label className="text-xs font-medium text-gray-600 mb-1 block">{t.volunteers.endLabel}</label>
